...
- A provider-native server using reserved ID `ralphx` or `ralphx_internal` fails launch preflight instead of being overwritten. The only compatibility exception is an exact historical user-scoped Claude `ralphx` registration written by RalphX: startup and launch preflight serialize revalidation, remove it through `claude mcp remove ralphx -s user`, and verify absence. Modified, ambiguous, project/local, Codex, and `ralphx_internal` collisions remain untouched and fail closed.
