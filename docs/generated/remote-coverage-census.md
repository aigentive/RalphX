...
> **P-11 is COMPLETE.** All 522 production invoke command names across `frontend/src` carry a reviewed disposition: remote-registered, reason-coded local-only, or manifest-classified (host-denied / v1-deferred / v1-audit-refused). **0 unclassified, 0 dynamic expressions, 0 suppressions.**
...
```
PASS: remote transport drift — 522 invoke command name(s), 0 dynamic, 0 seam bypasses; 277 manifest-classified; 0 unclassified (P-11 COMPLETE — permanent zero).
      P-11 census: all 522 names have a reviewed disposition — 244 remote-registered, 33 reason-coded local-only, 245 manifest-classified only, 0 unclassified, 0 suppressions.
```

| Measure | Count | Source |
|---|---|---|
| Invoke command names in `frontend/src` | 522 | drift scan (AST) |
