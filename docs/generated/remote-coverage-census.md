...
> **P-11 is COMPLETE.** All 587 production invoke command names carry a reviewed disposition: remote-registered, reason-coded local-only, plugin-local by the `plugin:` prefix rule, or manifest-classified (host-denied / v1-deferred / v1-audit-refused). **0 unclassified, 0 dynamic expressions, 0 suppressions.**
...
```
PASS: remote transport drift — 587 invoke command name(s), 0 dynamic, 0 seam bypasses; 284 manifest-classified; 0 unclassified (P-11 COMPLETE — permanent zero).
      P-11 census: all 587 names have a reviewed disposition — 251 remote-registered, 33 reason-coded local-only, 51 plugin-local (prefix rule), 252 manifest-classified only, 0 unclassified, 0 suppressions.
...
|---|---|---|
| Invoke command names on the transport | 587 | drift scan (AST over `frontend/src` + imported `@tauri-apps/plugin-*`) |
...
| Ledger rows (exhaustive over `generate_handler!`) | 580 | `docs/generated/remote-commands.json` |
