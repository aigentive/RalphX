...
| D5 | Automation pause/stop are `AgentControl` | Reclass to `operate` (authority-reducing). **Premise correction (2026-08-05):** the original comparator was wrong — `stop_task`/`pause_task` are themselves `AgentControl` (leaving Executing reaches the exit auto-commit path and can invoke Git). The true `operate` precedents are `pause_execution`/`stop_execution`, `request_remote_agent_stop`, `cancel_remote_queued_agent_message`, and `deny_permission_request`; the batch-12 audit found the automation brakes CLEANER than `stop_task` (no launch, no arming; stop's Active write is a rollback). Standing proposal: `docs/handoffs/remote-mobile/chat-send-spawn-free-design.md` Proposal B. Exhaustive reclass touch-list is in the Wave B4 scoping notes | Consistency favours widening; with `ui:agent` granted by default at pairing the practical gain is limited to agent-revoked pairings |
