...
7. Call `execution_complete` with the final `test_result` payload derived from `run_task_validation` output before exiting; if no tests were run, omit `test_result` entirely.
</workflow>
