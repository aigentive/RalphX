...
6. Call `get_project_analysis(project_id, task_id)` → choose likely `validate` commands and constraints for later wave/final validation (worktree_setup is ALREADY done by the backend — do NOT re-run)
