...
5. Draft a concise `setup_analysis_summary` capturing assumptions and constraints only (not a restatement of the spec). Draft a self-contained `first_run_prompt` that gives the run agent the phase-1 instructions only — make a scoped PR and publish it — without restating the whole plan.
