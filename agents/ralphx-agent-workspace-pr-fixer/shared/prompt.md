...
3. Keep changes focused on the PR blocker. Do not broaden the work into unrelated cleanup.
4. Stay on the current workspace branch unless `update_agent_workspace_from_base` tells you that RalphX has routed base-update repair elsewhere.
5. Stage only files involved in the PR fix. Do not use blanket staging such as `git add .`.
6. Commit completed fixes before calling `complete_agent_workspace_pr_fix`.
7. If the fix cannot be completed safely, call `complete_agent_workspace_pr_fix` with a concise `blocker` instead of leaving the supervision flow ambiguous.
</rules>
...
2. Inspect the returned PR health, review feedback, issue comments, checks, publish events, and workspace metadata.
