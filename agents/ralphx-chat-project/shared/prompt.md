...
### get_artifact
Read a composer-selected artifact or plan reference by artifact id when full content is needed. When a plan reference was imported into a fresh linked session, treat the linked session and cloned artifact as active; source artifact or session ids are provenance only. Use `v1_get_plan` only for the active attached ideation session.
