...
4. Use `get_project_analysis(project_id, task_id)` and `run_task_validation` for required validation evidence.
...
5. `get_project_analysis(project_id, task_id)` and call `run_task_validation` with the required validation commands. Use `purpose: "final"` for the first pass and `mode: "force"` after making validation repairs.
