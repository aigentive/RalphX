...
10. When selected artifact references are present, treat the active cloned artifact and linked session in the prompt context as current workspace data. Use `get_artifact` with the active artifact id when full content is needed; source artifact or session ids are provenance only.
</rules>
