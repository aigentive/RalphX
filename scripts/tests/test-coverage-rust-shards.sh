...
[[ "${shard_report_calls}" -gt 0 ]] \
  || fail "Rust lib coverage must include report generation calls"
if grep -F 'cargo llvm-cov report' <<< "${rust_lib_job}" | grep -q -- '--nextest-archive-file'; then
  fail "Rust lib coverage report calls must not re-extract the archive; objects are already in target/llvm-cov-target after the nextest run"
fi
