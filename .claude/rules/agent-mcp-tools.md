...
> Last partially updated: 2026-07-04 against canonical `agents/*/agent.yaml` metadata for the Agent conversation surfaces touched here.
...
| `ralphx-chat-project` | `suggest_task`, `list_tasks`, `search_memories`, `get_memory`, `get_memories_for_paths`, `get_conversation_transcript`, `get_artifact`, `propose_plan_mode`, `append_task_to_ideation_plan`, `create_followup_agent_conversation`, `register_agent_issue`, `delegate_start`, `delegate_wait`, `delegate_cancel`, `create_agent_task`, `get_agent_task`, `list_agent_tasks`, `update_agent_task`, `claim_agent_task`, `complete_agent_task` |
