...
| Delegation park/wake | Park records are durable, generation-scoped, and deadline-bounded; wake is a hidden `resume_in_place` message dispatched only after `commit_terminal` accepts and a `claim_wake` CAS succeeds; parent terminal paths CAS-disarm parks without cancelling delegates |
