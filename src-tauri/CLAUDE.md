...
| Review-owned worktree exclusion | While the Workspace Review monitor is `Reviewing` or a review fixer is active (`workspace_review_owns_worktree`), PR autofix dispatch, poller base updates, and the unattended base-update scans withhold with **zero durable writes** instead of mutating the worktree; every gate read fails closed. Manual Workspace Review start rejects while the workspace conversation has an active run. ❌ post-reservation `DeferredQueued` / `DeferToAgent` holds — they stamp misleading summaries, emit per-tick `deferred` events, and drop base-update assignments to a fixer that is itself withheld |
