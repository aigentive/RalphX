...
/// `execution_complete` only accepts the worker signal. The post-stream finalizer
/// must not publish terminal completion before its task transition succeeds.
#[tokio::test]
async fn test_execution_complete_does_not_emit_external_event_before_finalization() {
    let state = setup_test_state().await;
    ...
    // The authoritative finalizer has not run yet, so no completion event may exist.
    let events = state
        .app_state
    ...
    assert!(
        ...
        "task:execution_completed must wait for authoritative finalization"
    );
}
