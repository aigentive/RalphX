...
async fn sqlite_repo_persists_clickup_integration_settings() {
    ...
    assert_eq!(stored.branch_name_template, "work/:taskId:_:taskName:");
    assert_eq!(stored.commit_subject_template, ":taskId: | :summary:");
    assert_eq!(stored.pr_title_template, ":taskId: | :taskName:");
}
...
async fn sqlite_repo_returns_defaults_and_preserves_invalid_state() {
    ...
    assert!(!default_settings.strict_git_naming_enabled);
    assert_eq!(
        default_settings.branch_name_template,
