...
use ralphx_lib::application::chat_service::{
    ...
    format_agent_workspace_prompt_context, format_attachments_for_agent, format_session_history,
    get_entity_status_for_resume, is_text_file, persona_builder_requires_live_draft_session,
...
async fn codex_legacy_verification_session_uses_active_ideation_features() {
    ...
    assert!(
        ...
        "legacy verification sessions must use the active ideation Codex features: {args:?}"
    );
    assert!(
        !args.iter().any(|arg| arg == "features.shell_tool=false"),
        "legacy verifier features must not override active ideation routing: {args:?}"
    );
    let rendered_args = args.join("\n");
    assert!(
        rendered_args.contains("--agent-type") && rendered_args.contains("ralphx-ideation"),
        "legacy verification sessions must launch the active ideation agent: {args:?}"
    );
    assert!(
        !rendered_args.contains("ralphx-plan-verifier"),
        "legacy verifier routing must not override the active ideation agent: {args:?}"
    );
