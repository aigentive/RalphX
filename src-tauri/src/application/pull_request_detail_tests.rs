...
async fn repo_unresolvable_when_project_working_dir_is_unsafe() {
    ...
    assert_eq!(detail.state, PullRequestDetailState::RepoUnresolvable);
    assert_eq!(harness.github.state().fetch_github_connection_status_calls, 0);
}
...
async fn owned_inbound_pr_number_uses_source_pull_request_with_live_comments() {
    ...
    harness
        ...
        .await;
    harness.github.will_return_pr_detail(pr_detail(44, "review/source"));
    harness.github.state().fetch_pr_health_result =
        Some(Ok(health(Vec::new(), vec![issue_comment("c44", "review note")])));
...
async fn plan_branch_pr_number_uses_plan_origin() {
    ...
    harness.github.will_return_pr_detail(pr_detail(45, "plan/branch"));
    ...
    assert_eq!(detail.description.as_ref().unwrap().head_ref_name, "plan/branch");
}

#[tokio::test]
async fn branch_resolution_prefers_inbound_then_plan_branch_before_live_lookup() {
    let inbound = Harness::new().await;
    inbound.github.will_be_authenticated("github.com", "adriandemian");
    inbound
        .seed_source_workspace("44444444-4444-4444-4444-444444444444", "review/head", 46)
        .await;
    inbound.github.will_return_pr_detail(pr_detail(46, "review/head"));
    ...
    assert_eq!(inbound.github.state().find_latest_pr_by_head_branch_calls, 0);

    let plan = Harness::new().await;
    plan.github.will_be_authenticated("github.com", "adriandemian");
    plan.seed_plan_branch("plan/head", 47).await;
    plan.github.will_return_pr_detail(pr_detail(47, "plan/head"));
...
async fn best_effort_source_failures_are_reported_without_failing_description() {
    ...
    harness.github.will_be_authenticated("github.com", "adriandemian");
    harness
        .seed_workspace("55555555-5555-5555-5555-555555555555", "feature/soft", Some(55))
        .await;
    harness.github.will_return_pr_detail(pr_detail(55, "feature/soft"));
    harness.github.state().fetch_pr_health_result =
        Some(Err(AppError::Infrastructure("checks unavailable".to_string())));
    harness
        .github
        .will_fail_pr_review_thread("review thread unavailable");
    harness.github.state().check_pr_review_feedback_result =
        Some(Err(AppError::Infrastructure("feedback unavailable".to_string())));
...
async fn review_feedback_and_thread_comments_are_mapped_into_detail() {
    ...
    harness.github.will_be_authenticated("github.com", "adriandemian");
    harness
        .seed_workspace("66666666-6666-6666-6666-666666666666", "feature/review", Some(66))
        .await;
    ...
    assert_eq!(detail.review_thread[0].in_reply_to_id.as_deref(), Some("parent-1"));
    assert!(detail.review_thread[0].is_outdated);
    let summary = detail.review_summary.expect("review summary should be present");
    assert_eq!(
...
async fn status_labels_include_draft_and_merged() {
    ...
    draft.github.will_be_authenticated("github.com", "adriandemian");
    draft
        .seed_workspace("77777777-7777-7777-7777-777777777777", "feature/draft", Some(77))
        .await;
    draft
        .github
        .will_return_pr_detail(pr_detail_with_state(77, "feature/draft", PrStatus::Open, true));
    ...
    merged.github.will_be_authenticated("github.com", "adriandemian");
    merged
        .seed_workspace("88888888-8888-8888-8888-888888888888", "feature/merged", Some(88))
        .await;
