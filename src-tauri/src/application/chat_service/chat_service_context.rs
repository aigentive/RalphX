...
async fn build_launch_plan_for_harness_with_spawn_guard(
    ...
    if conversation_id != conversation.id.as_str() {
        return Err("conversation id does not match the conversation row".to_string());
    }
...
"#;
    ...
    async fn resume_commands_append_captured_attachment_context_for_claude_and_codex() {
        ...
        for (harness, cli_path, session_id) in harness_clis {
            ...
            assert!(
                mcp_args.contains("--conversation-id")
                    && mcp_args.contains("conversation-id")
                    && mcp_args.contains("--filesystem-enforced")
                    && mcp_args.contains("1"),
                "{harness} PersonaBuilder resume must carry the real conversation id and enforcement flag: {mcp_args}"
            );
            assert_eq!(
                spawnable_env_value(&spawnable, "RALPHX_FILESYSTEM_ENFORCED"),
                None,
                "{harness} filesystem enforcement must not use process env"
            );
    ...
    async fn codex_project_noninteractive_resume_keeps_current_conversation_id_for_mcp() {
        ...
        assert!(
            mcp_args.contains("--filesystem-enforced") && mcp_args.contains("\"1\""),
            "Codex queued PersonaBuilder resume must enable filesystem enforcement: {mcp_args}"
        );
        assert_eq!(
            spawnable_env_value(spawnable, "RALPHX_CONVERSATION_ID").as_deref(),
    ...
    async fn codex_project_noninteractive_resume_without_resume_capability_uses_recovery_exec() {
        ...
        assert!(
            mcp_args.contains("--conversation-id")
                && mcp_args.contains("codex-old-cli-conversation")
                && !mcp_args.contains("\"--conversation-id\",\"codex-project-old-resume\""),
            "Codex recovery MCP identity must be the conversation row id: {mcp_args}"
        );
        assert!(
            mcp_args.contains("--filesystem-enforced") && mcp_args.contains("\"1\""),
            "Codex PersonaBuilder recovery must enable filesystem enforcement: {mcp_args}"
        );
    }
