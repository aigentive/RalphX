...
use crate::domain::entities::{
    ...
    Artifact, ArtifactContent, ChatContextType, ChatConversation, ChatConversationId, MessageRole,
    Project,
};
...
async fn route_workspace_review_blocking_fixer_with_chat_service<S: ChatService + ?Sized>(
    ...
    let preserve_conversation_provider_session_ref = conversation
        .provider_session_ref()
        .is_some_and(|session_ref| Some(session_ref.harness) == runtime.harness);
    match chat_service
        .send_message(
            ChatContextType::Project,
            workspace.project_id.as_str(),
