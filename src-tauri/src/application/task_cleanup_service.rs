...
impl TaskCleanupService {
    ...
    /// Stop task runtimes and clean git resources without changing durable task rows.
    ///
    /// Use when another authority has already committed archival in the same transaction
    /// that owns the workflow state change.
    pub async fn cleanup_task_runtime_resources(
        &self,
        tasks: &[Task],
        stop_mode: StopMode,
    ) -> CleanupReport {
