...
use super::ideation_finalize_types::{ApplyProposalsInput, ApplyProposalsResult};
use crate::infrastructure::git_auth::{inspect_repository_capability, RepositoryCapability};

pub const SESSION_NO_LONGER_AWAITING_ACCEPTANCE: &str = "Session is no longer awaiting acceptance";

fn is_local_proposal(proposal: &TaskProposal, project_dir: &Path) -> bool {
    match &proposal.target_project {
        None => true,
        Some(target) => {
            let target = std::fs::canonicalize(target).unwrap_or_else(|_| PathBuf::from(target));
            target == project_dir
        }
    }
}
...
async fn ensure_base_branch_exists(
    ...
    GitService::create_branch(repo_path, base_branch, source)
        .await
        .map_err(|error| {
            format!("Failed to create branch '{base_branch}' from '{source}': {error}")
        })?;
    Ok(true)
}
...
fn finalize_session_acceptance(
    ...
    if rows != 1 {
        return Err(AppError::Validation(if require_pending_confirmation {
            SESSION_NO_LONGER_AWAITING_ACCEPTANCE.to_string()
        } else {
