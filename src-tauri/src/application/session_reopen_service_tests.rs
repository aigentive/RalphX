...
impl crate::domain::repositories::IdeationSessionRepository for FailingResetSessionRepo {
    ...
    async fn update_proposal_generation_progress(
        &self,
        session_id: &str,
        progress: crate::domain::entities::ProposalGenerationProgress,
    ) -> crate::error::AppResult<()> {
        self.inner
            .update_proposal_generation_progress(session_id, progress)
            .await
    }

    async fn reset_proposal_generation_progress(
        &self,
        session_id: &str,
    ) -> crate::error::AppResult<()> {
        self.inner
            .reset_proposal_generation_progress(session_id)
            .await
    }
