...
#[cfg(test)]
#[path = "sqlite_agent_conversation_jira_issue_repo_tests.rs"]
mod tests;
...
fn row_to_link(row: &rusqlite::Row<'_>) -> rusqlite::Result<AgentConversationJiraIssueLink> {
    ...
    Ok(AgentConversationJiraIssueLink {
        ...
        acceptance_criteria_markdown: row.get("acceptance_criteria_markdown")?,
        acceptance_criteria_text: row.get("acceptance_criteria_text")?,
        comments_json: row.get("comments_json")?,
        attachments_json: row.get("attachments_json")?,
        last_refreshed_at,
        refresh_status: AgentConversationJiraRefreshStatus::from_str(&refresh_status)
            .unwrap_or(AgentConversationJiraRefreshStatus::NotLoaded),
...
pub struct SqliteAgentConversationJiraIssueRepository {
    db: DbConnection,
}

impl SqliteAgentConversationJiraIssueRepository {
...
fn bind_link(
    ...
    statement.execute(params![
        ...
        link.acceptance_criteria_markdown.as_deref(),
        link.acceptance_criteria_text.as_deref(),
        &link.comments_json,
...
#[async_trait]
impl AgentConversationJiraIssueRepository for SqliteAgentConversationJiraIssueRepository {
    async fn get_by_conversation_id(
        ...
        self.db
            .run(move |conn| {
                conn.query_row(
                    "SELECT * FROM agent_conversation_jira_issue_links
            ...
            .run(move |conn| {
                ...
                let mut statement = conn.prepare(
                    "INSERT INTO agent_conversation_jira_issue_links (
                        conversation_id, project_id, provider, issue_key, issue_id, issue_url,
                        title, status, assignee, reporter, updated_at_remote,
                        description_markdown, description_text,
                        acceptance_criteria_markdown, acceptance_criteria_text,
                    ...
                    ON CONFLICT(conversation_id) DO UPDATE SET
                        ...
                        acceptance_criteria_markdown=excluded.acceptance_criteria_markdown,
                ...
                conn.query_row(
                    "SELECT * FROM agent_conversation_jira_issue_links WHERE conversation_id = ?1",
    ...
    async fn insert_if_absent(
        ...
        self.db
            .run(move |conn| {
                ...
                let mut statement = conn.prepare(
                    "INSERT OR IGNORE INTO agent_conversation_jira_issue_links (
                        conversation_id, project_id, provider, issue_key, issue_id, issue_url,
                        title, status, assignee, reporter, updated_at_remote,
                        description_markdown, description_text,
                        acceptance_criteria_markdown, acceptance_criteria_text,
    ...
    async fn clear(&self, conversation_id: &ChatConversationId) -> AppResult<()> {
        ...
        self.db
            .run(move |conn| {
                conn.execute(
                    "DELETE FROM agent_conversation_jira_issue_links WHERE conversation_id = ?1",
                    params![conversation_id],
                )?;
                Ok(())
            })
