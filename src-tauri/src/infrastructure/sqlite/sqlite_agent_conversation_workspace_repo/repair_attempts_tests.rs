...
#[tokio::test]
async fn deferred_ci_rerun_fields_round_trip_through_sqlite_transition() {
    let (_db, repo, conversation_id) = setup_repo();
    repo.create_or_update(workspace(conversation_id.clone()))
        .await
        .expect("persist workspace");

    let deferred_since = Utc::now();
    let mut deferred = repair_attempt(conversation_id.clone());
    deferred.ci_rerun_count = 1;
    deferred.ci_rerun_fingerprint = Some("ci-fingerprint".to_string());
    deferred.ci_rerun_pending_run_id = Some(123);
    deferred.ci_rerun_deferred_since = Some(deferred_since);
    let started = repo
        .start_or_join_repair_attempt(StartOrJoinAgentWorkspaceRepairAttempt {
            attempt: deferred,
            reason: "workflow is still active".to_string(),
            verified_newer_base: false,
            compatibility_projection: None,
            events: Vec::new(),
        })
        .await
        .expect("start deferred repair attempt");
    let StartOrJoinAgentWorkspaceRepairAttemptOutcome::Started(started) = started else {
        panic!("deferred repair generation must start");
    };
    let inserted = repo
        .get_current_repair_attempt(&conversation_id)
        .await
        .expect("reload inserted deferred attempt")
        .expect("deferred attempt exists");
    assert_eq!(inserted.ci_rerun_pending_run_id, Some(123));
    assert_eq!(inserted.ci_rerun_deferred_since, Some(deferred_since));

    let mut cleared = started.clone();
    cleared.phase = AgentWorkspaceRepairPhase::Ready;
    cleared.ci_rerun_pending_run_id = None;
    cleared.ci_rerun_deferred_since = None;
    cleared.updated_at += Duration::microseconds(1);
    let transitioned = repo
        ...
        .await
        .expect("clear deferred fields through SQLite CAS");
    assert!(matches!(
        transitioned,
        AgentWorkspaceRepairAttemptTransitionOutcome::Applied(ref attempt)
            if attempt.ci_rerun_pending_run_id.is_none()
                && attempt.ci_rerun_deferred_since.is_none()
                && attempt.ci_rerun_fingerprint.as_deref() == Some("ci-fingerprint")
    ));
    let reloaded = repo
        .get_current_repair_attempt(&conversation_id)
        .await
        .expect("reload cleared deferred attempt")
        .expect("cleared attempt exists");
    assert_eq!(reloaded.phase, AgentWorkspaceRepairPhase::Ready);
    assert_eq!(reloaded.ci_rerun_pending_run_id, None);
    assert_eq!(reloaded.ci_rerun_deferred_since, None);
    assert_eq!(
        reloaded.ci_rerun_fingerprint.as_deref(),
        Some("ci-fingerprint")
    );
}
