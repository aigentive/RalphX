...
impl SqliteValidationRunRepository {
    ...
    fn row_to_run(row: &rusqlite::Row<'_>) -> rusqlite::Result<ValidationRun> {
        ...
        Ok(ValidationRun {
            ...
            purpose: ValidationPurpose::from_str(row.get::<_, String>(3)?.as_str()),
            context_type: ValidationContextType::from_str(row.get::<_, String>(4)?.as_str()),
            requested_by_agent: row.get(5)?,
            status: ValidationRunStatus::from_str(row.get::<_, String>(6)?.as_str()),
            mode: ValidationRunMode::from_str(row.get::<_, String>(7)?.as_str()),
    ...
    fn row_to_command(row: &rusqlite::Row<'_>) -> rusqlite::Result<ValidationCommandResult> {
        ...
        Ok(ValidationCommandResult {
            ...
            command_source: ValidationCommandSource::from_str(row.get::<_, String>(4)?.as_str()),
            command_ref: row.get(5)?,
            command: row.get(6)?,
            cwd: row.get(7)?,
            label: row.get(8)?,
            category: ValidationCommandCategory::from_str(row.get::<_, String>(9)?.as_str()),
            reason: row.get(10)?,
            related_files,
            cache_key: row.get(12)?,
            cache_decision: ValidationCacheDecision::from_str(row.get::<_, String>(13)?.as_str()),
            status: ValidationCommandStatus::from_str(row.get::<_, String>(14)?.as_str()),
