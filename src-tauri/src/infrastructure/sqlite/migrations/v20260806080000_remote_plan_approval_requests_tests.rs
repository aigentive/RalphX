use super::{helpers, v20260806080000_remote_plan_approval_requests};
use rusqlite::Connection;

#[test]
fn remote_plan_approval_migration_has_columns_indexes_round_trip_and_reapplies() {
    let conn = Connection::open_in_memory().expect("db");
    v20260806080000_remote_plan_approval_requests::migrate(&conn).expect("migrate");
    ...
    v20260806080000_remote_plan_approval_requests::migrate(&conn).expect("idempotent");
}
