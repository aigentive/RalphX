// Migration v20260617121800: agent conversation jira issue links

use rusqlite::{params, Connection};

use crate::domain::services::primary_jira_reference_from_composer_metadata;
...
pub fn migrate(conn: &Connection) -> AppResult<()> {
    conn.execute_batch(
        "CREATE TABLE IF NOT EXISTS agent_conversation_jira_issue_links (
        ...
        CREATE INDEX IF NOT EXISTS idx_agent_conversation_jira_issue_links_project_key
            ON agent_conversation_jira_issue_links(project_id, issue_key);
        CREATE INDEX IF NOT EXISTS idx_agent_conversation_jira_issue_links_conversation
            ON agent_conversation_jira_issue_links(conversation_id);",
    )
    ...
    let candidate_sql = if has_workspace_table {
        "SELECT c.id,
         ...
         WHERE (c.agent_mode IS NOT NULL OR w.conversation_id IS NOT NULL)
           ...
           AND NOT EXISTS (
                SELECT 1 FROM agent_conversation_jira_issue_links link
...
fn backfill_conversation(
    ...
    for (message_id, metadata, created_at) in messages {
        let Some(reference) = primary_jira_reference_from_composer_metadata(metadata.as_deref())
        else {
            continue;
        };
        ...
        conn.execute(
            "INSERT OR IGNORE INTO agent_conversation_jira_issue_links (
                conversation_id, project_id, provider, issue_key, issue_id, issue_url, title,
             ...
             ) VALUES (?1, ?2, 'atlassian', ?3, ?4, ?5, ?6, '[]', '[]', 'not_loaded', ?7, ?8, 0, ?7, ?7)",
            params![
                conversation_id,
                project_id,
                reference.issue_key,
