...
#[cfg(test)]
#[path = "sqlite_agent_remote_conversation_start_request_repo_tests.rs"]
mod tests;

const SELECT_COLUMNS: &str = "id, conversation_id, project_id, content, provider, model, effort, \
     mode, status, error_code, requested_by_device_id, agent_run_id, claimed_at, created_at, \
     updated_at";
...
fn row_to_request(row: &rusqlite::Row<'_>) -> rusqlite::Result<RemoteConversationStartRequest> {
    let status_text: String = row.get("status")?;
    let status = RemoteConversationStartStatus::from_str(&status_text).map_err(|error| {
        rusqlite::Error::FromSqlConversionFailure(
            8,
            rusqlite::types::Type::Text,
    ...
    let claimed_at = match claimed_at {
        Some(value) => Some(parse_rfc3339(&value, 12)?),
        None => None,
    };
    ...
    Ok(RemoteConversationStartRequest {
        id: row.get("id")?,
...
pub struct SqliteRemoteConversationStartRequestRepository {
    db: DbConnection,
}

impl SqliteRemoteConversationStartRequestRepository {
...
#[async_trait]
impl RemoteConversationStartRequestRepository for SqliteRemoteConversationStartRequestRepository {
    async fn create_start_request(
        ...
        self.db
            .run(move |conn| {
                conn.execute(
                    "INSERT INTO remote_conversation_start_requests (
                        id, conversation_id, project_id, content, provider, model, effort, mode,
                        status, error_code, requested_by_device_id, agent_run_id, claimed_at,
    ...
    async fn get_start_request(
        ...
        self.db
            .run(move |conn| {
                conn.query_row(
                    &format!(
                        "SELECT {SELECT_COLUMNS} FROM remote_conversation_start_requests \
                         WHERE id = ?1"
                    ),
                    params![id],
                    row_to_request,
        ...
        self.db
            .run_transaction(move |conn| {
                ...
                let candidate_id: Option<String> = conn
                    .query_row(
                        "SELECT id FROM remote_conversation_start_requests \
                ...
                let changed = conn.execute(
                    "UPDATE remote_conversation_start_requests \
                     SET status = 'starting', claimed_at = ?1, updated_at = ?1 \
    ...
    async fn complete_start_request(
        ...
        self.db
            .run(move |conn| {
                conn.execute(
                    "UPDATE remote_conversation_start_requests \
                     SET status = 'started', agent_run_id = ?1, updated_at = ?2 \
            ...
            .run(move |conn| {
                let changed = conn.execute(
                    "UPDATE remote_conversation_start_requests \
                     ...
                     WHERE status = 'starting' AND claimed_at IS NOT NULL \
                       AND claimed_at < ?2",
