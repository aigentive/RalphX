...
use crate::domain::services::learned_skill_substrate::{
    ...
    MemoryToProjectSkillPromotionService, ProjectSkillAgingStatus, ProjectSkillDistillationOrigin,
    ProjectSkillDistillerService, ProjectSkillEvidenceLevel, ProjectSkillImportApplyInput,
...
async fn project_skill_distillation_stages_skill_with_outcome_provenance() {
    ...
    let staged = service
        .stage_skill_from_outcome(StageProjectSkillFromOutcomeInput {
            ...
            compact_guidance:
                "Before approving merge recovery, check the validation failure class.".to_string(),
            body_markdown: "Use the validation log as evidence before repeating a failed merge."
                .to_string(),
...
async fn project_skill_distiller_records_memory_pipeline_role_origin() {
    ...
    let skill_repo = Arc::new(MemoryProjectSkillRepository::new());
    let project_id = ProjectId::from_string("project-1".to_string());
    let mut outcome = new_empty_task_outcome(project_id.clone(), "task_pipeline", "task", "task-1");
    outcome.status = TaskOutcomeStatus::Eligible;
