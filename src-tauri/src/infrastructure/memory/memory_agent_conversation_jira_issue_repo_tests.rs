...
use super::*;
use crate::domain::entities::{AgentConversationJiraRefreshStatus, ChatConversationId, ProjectId};
...
async fn upsert_preserves_created_at_while_replacing_assignment_snapshot() {
    let repo = MemoryAgentConversationJiraIssueRepository::new();
    let conversation_id = ChatConversationId::new();
    let first = link(conversation_id, "RX-42", assigned_at());
    ...
    replacement.refresh_status = AgentConversationJiraRefreshStatus::Loaded;
    ...
    assert_eq!(saved.issue_key, "RX-77");
    assert_eq!(saved.created_at, first_created_at);
    assert_eq!(loaded.issue_key, "RX-77");
...
async fn insert_if_absent_returns_existing_assignment_without_overwriting() {
    let repo = MemoryAgentConversationJiraIssueRepository::new();
    let conversation_id = ChatConversationId::new();
    let existing = link(conversation_id, "RX-42", assigned_at());
...
async fn clear_removes_existing_assignment_and_allows_new_assignment() {
    let repo = MemoryAgentConversationJiraIssueRepository::new();
    let conversation_id = ChatConversationId::new();
    repo.upsert(link(conversation_id, "RX-42", assigned_at()))
        .await
        .expect("insert first");
    ...
    assert_eq!(inserted.issue_key, "RX-77");
}
