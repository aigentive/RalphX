 ...
 */
export async function handleGetSessionTasks(args, context) {
    const sessionId = args.session_id;
    if (!sessionId) {
        return JSON.stringify({ error: "missing_argument", message: "session_id is required" }, null, 2);
    }
 ...
 */
export async function handleAppendTaskToPlan(args, context) {
    const sessionId = args.session_id;
    ...
    try {
        ...
        const response = await getBackendClient().post(`/api/external/sessions/${encodeURIComponent(sessionId)}/tasks`, context, body);
        return JSON.stringify(response.body, null, 2);
    }
    catch (err) {
        return handleError(err);
