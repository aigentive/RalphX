    ...
    server.setRequestHandler(ListToolsRequestSchema, async () => ({
        tools: [
            ...
            },
            {
                ...
                description: "Queue a visible model-native Verify Plan turn in the session's planning conversation. The active model chooses useful review lenses, may revise the linked plan, and records proof for the exact final artifact. Returns status: 'queued' | 'already_queued' | 'already_running' | 'already_verified' | 'no_plan'",
                inputSchema: {
                    type: "object",
                    properties: {
                        session_id: { type: "string", description: "Ideation session ID" },
            ...
            },
            {
                ...
                description: "Get exact-artifact Verify Plan status. Returns status ('unverified'|'queued'|'verifying'|'verified'|'failed'|'cancelled'), in_progress, current_artifact_id, verified_artifact_id, and the authoritative action run id/timestamps/error when available.",
                inputSchema: {
                    type: "object",
                    properties: {
                        session_id: { type: "string", description: "Ideation session ID" },
