...
export const PLAN_TOOLS: Tool[] = [
    ...
    inputSchema: {
      ...
      properties: {
        status: {
          ...
          description:
            "Terminal verification status. Use verified or needs_revision for normal completion, and skipped only for flows where skip is actually allowed.",
        },
        round: {
          type: "integer",
        ...
        convergence_reason: {
          ...
          description:
            "Why verification converged or exited. Recommended on every terminal call and required for the standard verifier cleanup flow.",
        },
        generation: {
          type: "integer",
          description:
  ...
  },
  {
    ...
    description:
      "Stop a running verification loop and skip to completion. Kills the verification child agent immediately, sets status to skipped with convergence_reason user_stopped, and unfreezes the plan artifact. Idempotent: returns success if no verification is in progress.",
    inputSchema: {
      type: "object",
      properties: {
        session_id: {
