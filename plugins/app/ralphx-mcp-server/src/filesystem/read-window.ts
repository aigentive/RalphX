...
export async function readLineWindow(
  absolutePath: string,
  opts: ReadLineWindowOptions
): Promise<LineWindow> {
  const windowEndTarget = Math.min(
    opts.endLine ?? Number.POSITIVE_INFINITY,
    opts.startLine + opts.maxLines - 1
  );
  ...
  try {
    while (scannedBytes < MAX_LINE_SCAN_BYTES) {
      const bytesToRead = Math.min(
        READ_CHUNK_BYTES,
        MAX_LINE_SCAN_BYTES - scannedBytes
      );
  ...
  let stoppedBy: LineWindowStop;
  if (lines.length === 0) {
    stoppedBy = "out_of_range";
  } else if (stoppedAtMaxBytes) {
    stoppedBy = "max_bytes";
  } else {
    const finalRequestedLine = Math.min(
      opts.endLine ?? totalLines,
      totalLines
    );
    if (
      windowEnd === opts.startLine + opts.maxLines - 1 &&
...
export function formatReadHeader(
  window: LineWindow,
  opts: FormatReadHeaderOptions
): string[] {
  ...
  const isTruncated =
    window.stoppedBy === "max_bytes" || window.stoppedBy === "max_lines";
  let truncation = "TRUNCATED: false";
  if (isTruncated) {
    const reason =
      window.stoppedBy === "max_bytes"
        ? `max_bytes=${opts.maxBytes}`
        : `max_lines=${opts.maxLines}`;
    const requestedEndClause =
      opts.endLine !== undefined && window.windowEnd < opts.endLine
