type RuntimeStringContextKey =
  ...
  | "projectId"
  | "pipelineRole"
  | "skillDistillationBatchId"
  | "skillDistillationClaimToken"
  | "skillDistillationFingerprint"
  | "skillDistillationOutcomeIds"
  | "workingDirectory"
  | "filesystemReadRoots"
  | "contextType"
  | "contextId"
...
export function buildProjectSkillPipelineTransportHeaders(
  context: RuntimeContext
): Record<string, string> | undefined {
  const required = {
  ...
  return {
    ...
    ...(context.skillDistillationClaimToken?.trim()
      ? {
          "x-ralphx-skill-distillation-claim-token":
            context.skillDistillationClaimToken.trim(),
        }
      : {}),
    ...
    ...(context.skillDistillationOutcomeIds?.trim()
      ? {
          "x-ralphx-skill-distillation-outcome-ids":
            context.skillDistillationOutcomeIds.trim(),
        }
      : {}),
  };
