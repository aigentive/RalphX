...
describe("handleProposePlanMode", () => {
  ...
  it("returns a structured error when no conversation id is available", async () => {
    ...
    expect(parsedToolText(result)).toEqual({
      error: true,
      message:
        "Failed to propose plan mode: propose_plan_mode requires conversation_id because RalphX did not provide the current conversation id to the MCP runtime context.",
    });
    expect(fetchMock).not.toHaveBeenCalled();
  });
