...
export function createExclusionCounters() {
    return {
        gitignored: 0,
        hidden: 0,
        symlinks: 0,
...
export function recordExclusion(counters, kind, relativePath) {
    switch (kind) {
        case "gitignored":
...
export function formatExclusionNotes(counters, caps) {
    const notes = [];
    const pathExclusions = [];
    const includeFlags = [];
    if (counters.gitignored > 0) {
        pathExclusions.push(`${countLabel(counters.gitignored, "path")} excluded by .gitignore${formatSamples(counters.samples.gitignored)}`);
        includeFlags.push("respect_gitignore=false");
    }
    if (counters.hidden > 0) {
        pathExclusions.push(`${countLabel(counters.hidden, "hidden path")}${formatSamples(counters.samples.hidden)}`);
    ...
    if (pathExclusions.length > 0) {
        const inclusionHint = includeFlags.length > 0
    ...
    if (counters.resultCapReached && caps.maxResults !== undefined) {
        capNotes.push(`result cap reached (max_results=${caps.maxResults}); more matches exist`);
    }
    if (counters.entryCapReached && caps.maxEntries !== undefined) {
        capNotes.push(`entry cap reached (max_entries=${caps.maxEntries}); more entries exist`);
    }
    ...
    if (counters.oversizeFiles > 0 && caps.maxFileBytes !== undefined) {
        traversalLimits.push(`${countLabel(counters.oversizeFiles, "file")} skipped for exceeding max_file_bytes=${caps.maxFileBytes}${formatSamples(counters.samples.oversize)}`);
    }
    if (counters.depthTruncatedDirs > 0 && caps.maxDepth !== undefined) {
        traversalLimits.push(`${countLabel(counters.depthTruncatedDirs, "directory", "directories")} not descended at max_depth=${caps.maxDepth}`);
    }
...
function recordSample(samples, relativePath) {
