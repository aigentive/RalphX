...
export const IDEATION_TOOLS = [
        ...
        inputSchema: {
            ...
            properties: {
                ...
                depends_on: {
                    type: "array",
                    items: { type: "string" },
                    description: "Optional proposal IDs that must be completed before this proposal. Use for staged or ordered work; omit only when the proposal is genuinely independent.",
                },
        ...
        inputSchema: {
            ...
            properties: {
                ...
                affected_paths: {
                    type: "array",
                    items: { type: "string" },
                    description: "Updated coarse planned scope for the proposal. Use repo-relative path prefixes that bound the intended implementation area without pretending to know every final file.",
                },
                target_project: {
                    type: "string",
                ...
                add_depends_on: {
                    type: "array",
                    items: { type: "string" },
                    description: "Optional proposal IDs that this proposal should depend on. Adds dependency edges without replacing existing dependencies.",
                },
                add_blocks: {
                    type: "array",
                    items: { type: "string" },
                    description: "Optional proposal IDs that this proposal should block. Adds reverse dependency edges without replacing existing dependencies.",
                },
            },
    ...
    },
    {
        ...
        description: "Delete a task proposal. Alias for archive_task_proposal — routes to the same endpoint. Use when the user or agent wants to delete/remove a proposal during ideation.",
        inputSchema: {
            type: "object",
            properties: {
                proposal_id: {
    ...
    // ========================================================================
    {
        ...
        description: "Get the current acceptance_status for an ideation session. Use this to poll whether the user has accepted or rejected a pending finalize confirmation. " +
    ...
    },
    {
        ...
        description: "Get all ideation sessions that have a pending acceptance confirmation for the active project. " +
    ...
    // ========================================================================
    {
        ...
        description: "Ask the user one clarifying question, or provide questions[] to run a short interview from a single tool call. " +
        ...
        inputSchema: {
            ...
            properties: {
                session_id: {
                    type: "string",
                    description: "The ideation session ID when explicitly provided. If omitted, RalphX uses the current runtime conversation/session context.",
                },
                question: {
                    type: "string",
                ...
                metadata: {
                    type: "object",
                    description: "Optional UI metadata for RalphX-owned question affordances. Use sparingly for structured proposal flows.",
        ...
        inputSchema: {
            ...
            properties: {
                source_ideation_session_id: {
                    type: "string",
                    description: "Optional explicit ideation session to follow up from. When omitted and source_task_id is provided, the tool resolves the correct local parent session from the task automatically.",
                },
                title: {
                    type: "string",
    ...
    },
    {
        ...
        inputSchema: {
            ...
            properties: {
                parent_session_id: {
                    type: "string",
                    description: "Optional legacy explicit parent ideation session. Omit this in normal agent contexts; RalphX infers parent context from the MCP transport.",
                },
                parent_turn_id: {
                    type: "string",
                ...
                parent_conversation_id: {
                    type: "string",
                    description: "Optional parent conversation id for linking the delegated conversation back to the invoker chat. Normally supplied by the MCP transport.",
                },
                parent_tool_use_id: {
                    type: "string",
        ...
        inputSchema: {
            ...
            properties: {
                ...
                job_ids: {
                    type: "array",
                    items: { type: "string" },
                    description: "Watch a whole delegated wave with one call; returns as soon as any listed job settles. Provide either job_id or job_ids, not both.",
                },
                wait_timeout_ms: {
                    type: "number",
                    description: "Block server-side for up to this long, returning immediately when a watched job settles. Omit for the legacy immediate-return snapshot. Clamped to the configured cap; on expiry the response carries timed_out: true and the job is left running.",
                },
    ...
    {
        name: "delegate_park",
        description: "Register a durable wake set for RalphX-native delegated specialist jobs and explicitly END YOUR TURN. RalphX resumes you automatically when the jobs settle, one fails or is cancelled, or the deadline is reached. Use this instead of repeated delegate_wait polling for long waits.",
        inputSchema: {
            type: "object",
            properties: {
                job_ids: {
    ...
    },
    {
        ...
        description: "Fetch older chat messages for an ideation session. The session bootstrap already includes the NEWEST messages — use this tool only when you need earlier history beyond what was provided. " +
...
//# sourceMappingURL=ideation-tools.js.map