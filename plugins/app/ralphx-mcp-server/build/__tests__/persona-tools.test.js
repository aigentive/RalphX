...
import { PERSONA_BUILDER_TOOLS, callPersonaTool, isPersonaToolName, } from "../persona-tools.js";
function captureCalls() {
    ...
    const callTauriGet = async (path, options) => {
...
describe("persona builder MCP tools", () => {
    ...
    it("posts the Rust request shape and caller conversation header when saving", async () => {
        ...
        await callPersonaTool("save_persona_draft", callTauri, callTauriGet, {
            draftId: "draft-1",
            slug: "researcher",
    ...
    it("gets a persona draft through the GET route with caller context", async () => {
        ...
        await callPersonaTool("get_persona_draft", callTauri, callTauriGet, { draft_id: "draft-1" }, { conversationId: "conversation-1" });
