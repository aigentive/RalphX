export const PERSONA_BUILDER_TOOLS = [
    {
        name: "save_persona_draft",
        description: "Create or update a persona draft owned by the current PersonaBuilder conversation. " +
...
export function isPersonaToolName(name) {
    return PERSONA_TOOL_NAMES.has(name);
}
export async function callPersonaTool(name, callTauri, callTauriGet, args, runtimeContext) {
...
function personaToolHeaders(toolName, runtimeContext) {
    const conversationId = runtimeContext?.conversationId?.trim() ?? "";
    if (conversationId.length === 0) {
        throw new Error(`${toolName} requires the current PersonaBuilder conversation id from the RalphX MCP runtime context.`);
    }
    return {
        [CALLER_SESSION_ID_HEADER]: conversationId,
    };
}
function savePersonaDraftPayload(args) {
