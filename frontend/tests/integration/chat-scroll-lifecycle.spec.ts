...
test.describe("existing conversation bottom follow", () => {
  ...
  test("keeps Virtuoso the only scroll writer and never moves the reader up the transcript", async ({ page }, testInfo) => {
    ...
    // The controller issues no raw scroll writes at all. One measured corrective
    // write under followOutput took scrollError from 119 to 228 with a 473px jump.
    expect(recorded.filter(({ fromController }) => fromController)).toEqual([]);
    // A bottom target computed from a transiently under-reported extent used to
  ...
  test("leaves a reader who scrolled away where they are", async ({ page }, testInfo) => {
    ...
    // The reader must never be dragged down. Distance to the bottom is only
    // asserted while the turn is still adding content: completing a run drops
    // the live streaming row, and the browser legitimately clamps a parked
    // reader onto a transcript that became shorter than their position.
    const expectStillParked = async (step: string, expectAwayFromBottom = true): Promise<void> => {
      const { clientHeight, scrollHeight, scrollTop } = await chat.geometry();
      expect(scrollTop, `${step} moved the parked reader down`)
        .toBeLessThanOrEqual(parked + PARKED_TOLERANCE_PX);
      if (!expectAwayFromBottom) return;
      expect(scrollHeight - clientHeight - scrollTop, `${step} returned the reader to the bottom`)
